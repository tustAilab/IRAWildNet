...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio=3, drop_path=0.):
        super().__init__()
        self.dwconv_7x7 = ConvBN(dim, dim, 7, 1, (7 - 1) // 2, groups=dim, with_bn=True)
        self.dwconv_5x5 = ConvBN(dim, dim, 5, 1, ( 5- 1) // 2, groups=dim, with_bn=True)
        ...
        self.g = ConvBN(mlp_ratio * dim*2, dim, 1, with_bn=True)
    ...
    def forward(self, x):
        ...
        x7,x51,x52,x3= self.f1(x_1),self.f2(x_2),self.f3(x_2),self.f4(x_3)
        x1=self.act(x7) * self.act(x51)
        x2 = self.act(x52) * self.act(x3)
